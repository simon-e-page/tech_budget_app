...
class Account(AttributeToKey):
  ...
  def get_balance(self, as_date=None):
    if as_date is None:
      balance = anvil.server.call('get_account_balance', self.name)
    else:
      balance = anvil.server.call('get_starting_balance', self.name, as_date)
    return balance
  ...
  def is_fully_reconciled(self):
    return anvil.server.call('is_fully_reconciled', self.name)
...
class Accounts(AttributeToDict):
  ...
  def match_filename(self, filename):
    ...
    account_name = anvil.server.call('get_account_from_filename', filename)
    return account_name

  def test_reconciled(self):
    return anvil.server.call('test_reconciled')

  def get_accounts_by_month(self, end, compare='budget', start=None, account_names=None, fill=True):
    return anvil.server.call("get_accounts_by_month", end, compare, start=start, account_names=account_names, fill=fill)
    ...
    if icon_id is None:
      try:
        ...
        if content.length > 100000:
          ...
          icon_id = anvil.server.call('create_account_icon_file', name, content)
        else:
...
class Rule(AttributeToKey):
  ...
  def save(self):
    ...
    if self.id is None:
      try:
        self.id = anvil.server.call('add_rule', self.pattern_description, self.account_name, **props)    
        ret = self.id
      except Exception as e:
...
class Importer:
  def start(self, file_obj, account_name):
    return anvil.server.call('upload_transactions', file_obj, account_name)

  def next_batch(self, code):
    return anvil.server.call('upsert_batch', code)

  def process_paypal(self, account_name, start, end):
    return anvil.server.call('process_paypal', account_name, start=start, end=end)
...
def gpt_set_account_data(account_name):
  return anvil.server.call('gpt_set_account_data', account_name=account_name)

def gpt_run(prompt):
  return anvil.server.call('gpt_run', prompt=prompt)
