...
class Users(UsersTemplate):
  ...
  def form_refreshing_data_bindings(self, **event_args):
    ...
    if (not self.initialised and self.item is not None) or (
      self.transaction_copy == {}
    ):
  ...
  def disable_checkbox_change(self, **event_args):
    ...
    if not self.disable_checkbox.checked or confirm(
      "This will prevent this entry being used in future budgets. Are you sure?",
      large=True,
    ):
  ...
  def actual_button_click(self, **event_args):
    """This method is called when the button is clicked"""
    self.item.transaction_type = (
      "Actual" if self.item.transaction_type == "Budget" else "Budget"
    )
    self.transaction_entries_1.build_table(self.item)
    self.refresh_data_bindings()
