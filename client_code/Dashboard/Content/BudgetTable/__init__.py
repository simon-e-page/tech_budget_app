from ._anvil_designer import BudgetTableTemplate
from anvil import *
import anvil.server
import anvil.users
...
class BudgetTable(BudgetTableTemplate):
  ...
  def update_entries(self, vendor, entries):
    ...
    for transaction_id, trans_entries in entries.items():
      ...
      if transaction is None:
        print(
          f"Can't find transaction {transaction_id} associated with Vendor {vendor.name}"
        )
      else:
  ...
  def summary_table_table_built(self, **event_args):
    ...
    a_data = {"id": "CY Budget", "total": 0.0} | {
      x: 0.0 for x in self.year_months
    }
    ly_data = {"id": "LY Budget", "total": 0.0} | {x: 0.0 for x in self.year_months}
    ...
    dc_data = {"id": "Cumul Difference", "total": 0.0} | {
      x: 0.0 for x in self.year_months
    }
    pc_data = {"id": "Cumul Percentage", "total": 0.0} | {
      x: 0.0 for x in self.year_months
    }
    ...
    for i, year_month in enumerate(self.year_months):
      ...
      ac_data[year_month] = (
        a_data[year_month] + ac_data[self.year_months[i - 1]]
        if i > 0
        else a_data[year_month]
      )
      lc_data[year_month] = (
        ly_data[year_month] + lc_data[self.year_months[i - 1]]
        if i > 0
        else ly_data[year_month]
      )
